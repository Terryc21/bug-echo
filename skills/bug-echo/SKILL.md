...
2.1 **Check whether the removed lines are self-sufficient.**
   ...
   **If not self-sufficient**, do not read the whole enclosing scope. Most of a function has nothing to do with the line you fixed, and grabbing all of it is how this step turns into noise. Use the function as the *area to search*, then narrow it by what the line actually depended on:
   ...
   ⚠️ **The pass-through rule (step 3) is the least-tested part of this step. Watch it on real runs.** The one-step-plus-pass-through shape came from a domain where the analogous check never walks a chain at all, so it is the piece that transferred with the least evidence behind it. Two things to watch, and to say out loud when they happen:

   - **A pass-through you did not recognize.** `x = y` is the obvious form, but a wire can also look like `let x = y.value`, a trivial unwrap, a cast, or a one-line accessor. If step 2 lands on something that carries a value without transforming or checking it, that is a wire even if it does not match `x = y` literally. Stopping there yields a confident-looking precondition built on nothing, and **this is the one branch that can be wrong while looking precise.** When unsure whether a site is a wire, treat it as one and take the extra step.
   - **The cap being hit often.** Repeatedly reaching three steps suggests the walk is too shallow for this codebase, not that the code is unusual.

   **Report the outcome mix at the end of a run that used this step:** how many candidates resolved on the first step, how many needed the pass-through exception, how many fell back to the function. One line is enough. Nobody has measured this on a real codebase yet, and the mix is the evidence that decides whether the cap and the exception are set correctly.
